...
function bindDashboardControls() {
  ...
  $("#clients-grid")?.addEventListener("click", async (event) => {
    ...
    if (action.dataset.apiAction === "copy-id") {
      await copyText(apiId);
      toast("client_id 已复制");
      return;
    }
  ...
  $("#secret-dialog")?.addEventListener("click", async (event) => {
    ...
    <article class="item-card" data-api-id="${escapeHtml(client.id)}">
      <div class="item-head">
        <div>
          ...
          <div class="meta">应用名称 / Application Name</div>
        </div>
        <span class="badge">${escapeHtml(statusLabel(client.status))}</span>
      </div>

      <div class="meta">Client ID</div>
      <code class="client-id-line">${escapeHtml(client.id)}</code>

      <div class="meta">回调地址 / Redirect URI</div>
      <div class="redirect-list">
        ${redirectUris.length
          ? redirectUris.map((uri) => `<code class="client-uri">${escapeHtml(uri)}</code>`).join("")
          : `<span class="meta">-</span>`}
      </div>

      <div class="meta">权限范围 / Scopes</div>
      <div class="chip-grid">
        ${scopes.map((scope) => `<span class="chip static"><span>${escapeHtml(scope)}</span></span>`).join("")}
      </div>

      <div class="card-actions">
        <button class="ghost" data-api-action="copy-id">复制 client_id</button>
...
async function createClient(event) {
  ...
  $("#client-id-once").textContent = api.id || "";
  $("#client-secret").textContent = api.clientSecret || "";

  $("#copy-created-client-id")?.setAttribute("data-copy-value", api.id || "");
  $("#copy-created-client-secret")?.setAttribute("data-copy-value", api.clientSecret || "");
